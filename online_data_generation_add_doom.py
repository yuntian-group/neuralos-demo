...
# Preload desktop reference and doom icon assets
try:
    _DESKTOP_REF_BGR = cv2.imread('desktop.png', cv2.IMREAD_COLOR)
    if _DESKTOP_REF_BGR is None:
        logger.warning('desktop.png not found or unreadable; desktop detection will be disabled.')
except Exception as _e:
    logger.error(f'Failed to load desktop.png: {_e}')
    _DESKTOP_REF_BGR = None
...
try:
    ...
    logger.error(f'Failed to load doom_icon.png: {_e}')
    _DOOM_ICON = None
    _DOOM_ICON_HL = None

def detect_cursor_in_frame(frame_bgr: np.ndarray, white_thresh: int = 35, min_area: int = 50):
    """Detect cursor in the top-right 128x128 region; returns (patch, mask, (x,y)) or (None,None,None)."""
    try:
...
def is_desktop_frame(frame_bgr: np.ndarray, desktop_bgr: np.ndarray, mae_threshold: float = 12.0) -> bool:
    """Return True if frame is sufficiently similar to the reference desktop image using mean absolute error."""
...
def overlay_doom_icon_on_frame(frame_bgr: np.ndarray) -> np.ndarray:
    """Overlay doom icon on a BGR frame if assets available; highlight if cursor intersects icon rect; re-layer cursor on top."""
    if _DOOM_ICON is None:
        return frame_bgr
    # Detect cursor before modification
    cursor_patch, cursor_mask, cursor_pos = detect_cursor_in_frame(frame_bgr)
    # Decide highlight based on cursor intersecting icon rectangle
    highlight = False
    if cursor_patch is not None and cursor_pos is not None:
        px, py = cursor_pos
        h, w = cursor_patch.shape[:2]
        cur_rect = (px, py, px + w, py + h)
        ix1, iy1, ix2, iy2 = DOOM_ICON_RECT
        cx1, cy1, cx2, cy2 = cur_rect
    ...
    # Re-paste cursor on top if available
    if cursor_patch is not None and cursor_pos is not None:
        px, py = cursor_pos
        h, w = cursor_patch.shape[:2]
        ...
        for i, sub_traj in enumerate(sub_trajectories):
            ...
            try:
                ...
                with VideoFileClip(video_file) as video:
                    ...
                    # Prepare doom-modified video writer
                    doom_video_file = f'raw_data/raw_data/videos/record_{record_num}_doom.mp4'
                    first_frame_rgb = video.get_frame(0)
